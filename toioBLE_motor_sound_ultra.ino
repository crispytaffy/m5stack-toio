...
static boolean is_req_move_forward = false;
...
static void moveMotor(void)
{
  ...
  if(M5.BtnB.wasPressed()){
    M5.Lcd.drawString("PLAY", 90, 180);
    is_req_sound = true;
  }else{
    M5.Lcd.drawString("-", 90, 180);
    is_req_sound = false;
  }
}
...
static void sendMotorControl(void)
{
  ...
  data[2] = 0x01; //前
  data[3] = (is_req_move_forward ? 0x64 : 0x00); //モーターの速度指示値　0x64 = 100
  data[4] = 0x02; //右　motor id 2 : right
  data[5] = 0x02; //後
  data[6] = (is_req_move_forward ? 0x64 : 0x00); //モーターの速度指示値　0x1E = 30
...
//  data[2] = (is_req_sound ? 0x05 : 0x0); //Operationの数 7つ
  data[2] = (is_req_move_forward ? 0x04 : 0x0); //Operationの数 7つ
  ...
  data[14] = 0xFF; //音量 最大
//  data[15] = 0x1E; //再生時間 300ミリ秒
//  data[16] = 0x40; //MIDI note number E5
//  data[17] = 0xFF; //音量 最大
//  data[18] = 0x1E; //再生時間 300ミリ秒
//  data[19] = 0x3E; //MIDI note number D5
//  data[20] = 0xFF; //音量 最大
//  data[21] = 0x1E; //再生時間 300ミリ秒
//  data[22] = 0x3C; //MIDI note number C5
//  data[23] = 0xFF; //音量 最大
...
void loop() {
  ...
  if(fConnected) {
    if (M5.BtnA.wasPressed()) {
      uint8_t value = motor_characteristic->readUInt8();
